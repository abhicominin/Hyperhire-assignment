...
function parabola(twoPoints: any) {
  ...
  let step = 80; 
  let heightProportion = 0.125;
  let dLon = (twoPoints[2] - startPoint[0]) / step; 
  let dLat = (twoPoints[3] - startPoint[1]) / step; 
  let deltaLon = dLon * Math.abs(111000 * Math.cos(twoPoints[1])); 
  let deltaLat = dLat * 111000; 
  let endPoint = [0, 0, 0]; 
  let heigh =
    Number(
      (
  ...
  let x2 = 10000 * Math.sqrt(dLon * dLon + dLat * dLat);
  let a = heigh / (x2 * x2);
  ...
  for (var i = 1; i <= step; i++) {
    ...
    let x = x2 * ((2 * i) / step - 1); 
    endPoint[2] = Number(y(x, heigh).toFixed(0)) * 1; 
    s = s.concat(endPoint);
